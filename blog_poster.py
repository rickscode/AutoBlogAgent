...
WP_BASE_URL = os.getenv("WP_BASE_URL", "https://www.elcpp.com/wp-json/wp/v2")
...
CLOUDFLARE_API_URL = "https://api.cloudflare.com/client/v4/accounts/182569baf52ed215f0de16881064da51/ai/run/@cf/black-forest-labs/flux-1-schnell"
CLOUDFLARE_API_TOKEN = "0in_2jwxGoF4XtJWZdu7btLofnBHjU00zthofL6z"
PROMPT = "Image Promt"
OUTPUT_DIR = os.getcwd()  # Saves in the current directory
IMG_NAME = "ECLPP English IELTS Blog Image"
...
def gather_latest_english_learning_info():
    ...
    queries = [
        "Learn IELTS Online",
        "Learn English Online in Phnom Penh",
        # "General English program tips in Cambodia"
    ]
...
def generate_blog_post_content_and_title(latest_info):
    ...
    prompt = f"""
    You are a professional content writer creating a unique blog post for business name, an business type, in city, country.
    The business offers business offerings and services.

    Using the latest information provided:
    {latest_info}

    Write an engaging blog post that:
    - Mentions "Keywords," "Keywords," "Keywords," 
      and other relevant keywords for SEO.
    - Has a friendly and promotional tone to appeal to prospective customers.
    - Concludes with a call-to-action encouraging readers to visit business website for more details.

    Additionally, generate a unique and catchy blog post title that:
    - Includes "business name" and is SEO-friendly.
    - Reflects the blog post content.
    - Avoids repetition of previously generated titles (e.g., "").
    ...
    try:
        ...
        return "Business name Blog Post", "An error occurred while generating text."
...
def generate_and_save_image(prompt=None, output_dir=OUTPUT_DIR):
    ...
    default_prompt = "Generate a prompt for an image of"

    # Use Groq LLM to generate a more detailed prompt if no custom prompt is provided
    if not prompt:
        try:
            llm_prompt = """
            Generate a prompt for an image of.
            """
            response = client.chat.completions.create(
...
def main():
    ...
    try:
        image_path = generate_and_save_image(None, OUTPUT_DIR)
